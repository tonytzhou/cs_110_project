...
import { getFirestore, collection, addDoc, deleteDoc, doc, onSnapshot, query, orderBy, where, serverTimestamp, documentId } from 'firebase/firestore'
...
function toggleFavCount() {
  showFavCount.value = !showFavCount.value
}
...
const favoritesEmail = computed(() => route.params.email || userStore.currentUser)
...
function subscribeFeed() {
  if (route.name === 'UserFavorites') {
    ...
    if (!email) return
    const favQ = query(collection(db, 'favorites'), where('user', '==', email))
    onSnapshot(favQ, snap => {
      ...
      const chunks = postIds.length > 10 ? [postIds.slice(0, 10), postIds.slice(10)] : [postIds]
      const fetched = []
      chunks.forEach(chunk => {
        const pq = query(postsCol, where(documentId(), 'in', chunk), orderBy('timestamp', 'desc'))
        onSnapshot(pq, snap2 => {
  ...
  if (userStore.viewingUser) {
    const q = query(collection(db, 'posts'), where('user', '==', userStore.viewingUser), orderBy('timestamp', 'desc'))
    onSnapshot(q, snap => {
  ...
  userStore.fetchMyFollowingList().then(list => {
    ...
    if (list.length > 0 && list.length <= 10) {
      q = query(collection(db, 'posts'), where('user', 'in', list), orderBy('timestamp', 'desc'))
    } else {
      q = query(collection(db, 'posts'), orderBy('timestamp', 'desc'))
    }
    onSnapshot(q, snap => {
      const all = snap.docs.map(d => ({ id: d.id, ...d.data() }))
      posts.value = list.length > 0 && list.length <= 10 ? all : all.filter(p => list.includes(p.user))
    })
  })
}
...
<template>
  <div class="center_page">
    <div class="wrapper" ref="feedWrapper">
      ...
      <div
        v-else-if="route.name !== 'Favorites' &&
                   route.name !== 'UserFavorites' &&
                   !userStore.isLoggedIn"
        class="login_prompt"
      >
      ...
      <div v-for="post in posts" :key="post.id" class="user_post_box">
        ...
        <div class="favorite_controls">
          <button class="favorite_btn" @click="toggleFavorite(post)">
            <span v-if="showFavCount && favoritesMap[post.id]?.favorited">❇️</span>
            <span v-else-if="showFavCount && !favoritesMap[post.id]?.favorited">🟩</span>
          </button>
      ...
      <div v-if="route.name === 'UserFavorites' && posts.length === 0" class="no_posts">
        This user hasn’t favorited any posts yet.
      </div>
    </div>

    <div v-if="isOwnFavoritesView" class="side_buttons">
      <ScreenshotButton
